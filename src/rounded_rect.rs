...
use crate::{PathEl, Rect, Shape, Vec2, arc::{Arc, ArcAppendIter}};
...
impl RoundedRect {
    ...
    pub fn radius(&self) -> f64 {
        self.radius.abs().min(self.width().abs() / 2.0).min(self.height().abs() / 2.0)
    }
...
impl Shape for RoundedRect {
    ...
    fn to_bez_path(&self, tolerance: f64) -> RoundedRectPathIter {
        ...
        let radii = Vec2 { x: self.radius, y: self.radius };
        ...
        let arcs = [
            build_arc_iter(2, Vec2 { x: self.rect.x0 + radius, y: self.rect.y0 + radius }),
            build_arc_iter(3, Vec2 { x: self.rect.x1 - radius, y: self.rect.y0 + radius }),
            build_arc_iter(0, Vec2 { x: self.rect.x1 - radius, y: self.rect.y1 - radius }),
            build_arc_iter(1, Vec2 { x: self.rect.x0 + radius, y: self.rect.y1 - radius }),
        ];

        let rect = RectPathIter { rect: self.rect, ix: 0, radius: self.radius, };
...
impl Iterator for RectPathIter {
    ...
    fn next(&mut self) -> Option<PathEl> {
        self.ix += 1;
        match self.ix {
            1 => Some(PathEl::Moveto(Vec2::new(self.rect.x0, self.rect.y0 + self.radius))),
            2 => Some(PathEl::Lineto(Vec2::new(self.rect.x1 - self.radius, self.rect.y0))),
            3 => Some(PathEl::Lineto(Vec2::new(self.rect.x1, self.rect.y1 - self.radius))),
            4 => Some(PathEl::Lineto(Vec2::new(self.rect.x0 + self.radius, self.rect.y1))),
